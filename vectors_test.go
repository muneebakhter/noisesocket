...
func ValidateMessage(t *testing.T, message *Message, payload []byte) {
	assert.Equal(t, payload, mustHex(message.Payload))
	fields, err := parseMessageFields(payload)
	assert.NoError(t, err)
	assert.Equal(t, len(fields), len(message.Fields))

	for i, f := range fields {
		assert.Equal(t, f.Type, message.Fields[i].Type)
		assert.Equal(t, f.Data, mustHex(message.Fields[i].Data))
	}
